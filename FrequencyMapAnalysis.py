...
class CheckPoint():
    def __init__(self, parent, controller):
        ...
        title = tk.Label(self.win, text = 'Searching for files...',font=MEDIUM_FONT).grid(row=0,column=0,columnspan=2,pady=10,padx=10,sticky='news')
        ...
        for electrode in electrode_list:
            electrode_label = tk.Label(self.win, text = 'E%s' % electrode,font=SMALL_FONT).grid(row=row_value,column=0,pady=5,padx=5)
...
class InitializeFigureCanvas():
    ...
    def InitializeSubplots(self,ax,electrode):
        ...
        self.search_val = 0
        self.list_val = (electrode*3)
        try:
            freq = frequency_list[0]

            #-- retrieve the filename --#
            try:
                filename, filename2 = _retrieve_file(electrode, freq)
            except:
...
class ElectrochemicalAnimation(animation.FuncAnimation):
    ...
    def _quick_step(self):
        ...
        filename, filename2 = _retrieve_file(self.electrode, frequency)
    ...
    def Animate(self, args):
        ...
        if key > 0:
            while True:
                ...
                file, frequency_axis, charge_axis, potentials, adjusted_potentials, smooth_currents, adjusted_currents, regression, num, count = args
...
class RawGenerator():
    def __init__(self, num, ax):
        ...
        ###############################################################
        if e_var == 'single':
            self.list_val = (self.electrode*3)
        elif e_var == 'multiple':
            self.list_val = 3
    ...
    def Analyze(self, file, count, freq, method):
        ...
        ##############################
        filename, filename2 = _retrieve_file(self.electrode, freq)
        ...
        return self.file, self.frequency_axis, self.charge_axis, potentials, adjusted_potentials, smooth_currents, adjusted_currents, eval_regress, self.num, count
